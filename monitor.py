#!/usr/bin/env python3
"""
Monitor NARUTO CARD GAME (Bandai) — caza de primeras preventas/listados.

El juego de Bandai se lanza en 2027 (revelación completa 29-jul-2026, debut Gen Con
2026). Se venderá en las MISMAS tiendas que One Piece Card Game (son distribuidores
Bandai TCG). Mientras no salga, el bot vigila feeds de preventa / novedades / búsqueda
"naruto" en cada tienda y avisa en cuanto aparezca el primer producto.

Features:
- Filtro por keywords: solo notifica títulos que contengan "naruto" (required_keywords),
  descartando el viejo Naruto CCG y el "Naruto Mythos" no-Bandai (exclude_keywords).
- Prioridad de PRODUCTO: cases y booster boxes (high_value_keywords) se marcan 🚨 y se
  muestran primero — es lo que más interesa al usuario.
